{ config, inputs, lib, namespace, pkgs, system, ... }:
let
  inherit (lib) mkIf getExe getExe' mkForce;
      ...
      config = {
        ...
        in lib.mkMerge [
          (lib.mkOptionDefault {
            ...
            "${swayCfg.modifier}+Control+l" =
              mkForce "exec ${screen-locker} --immediate";
            ...
            XF86AudioRaiseVolume =
              "exec wpctl set-volume @DEFAULT_AUDIO_SINK@ 2.5%+";
          ...
          (lib.mkOptionDefault (builtins.listToAttrs (builtins.concatLists
            (builtins.genList (x:
              let
                ws = let c = (x + 1) / 10;
