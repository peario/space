...
in {
  options.${namespace}.suites.development = {
    ...
    kubernetesEnable =
      mkBoolOpt false "Enable kubernetes development configuration.";
  ...
  config = mkIf cfg.enable {
    home = {
      packages = with pkgs;
        [ jqp neovide onefetch postman vscode ]
        ++ lib.optionals pkgs.stdenv.isLinux [ github-desktop qtcreator ]
        ++ lib.optionals cfg.nixEnable [
          ...
          # ue4
          unityhub
        ] ++ lib.optionals cfg.sqlEnable [ dbeaver-bin mysql-workbench ];

      shellAliases = { prefetch-sri = "nix store prefetch-file $1"; };
    };
    ...
    space = {
      programs = {
        ...
        graphical = { editors = { vscode = enabled; }; };
