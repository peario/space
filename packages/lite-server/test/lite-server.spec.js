...
describe('lite-server', () => {
  ...
  beforeEach(() => {
    ...
    consoleStubs = {log : sinon.stub(), info : sinon.stub()};

    configDefaultsMock = {server : {middleware : [ 'm1', 'm2' ]}};
  ...
  it('should merge configs', () => {
    ...
    const bsConfigMock = {b : 2, server : {middleware : {0 : null}}};
  ...
  it('should handle missing bs-config', () => {
    ...
    const liteServer = require('../lib/lite-server');
    const bs = liteServer(
        {console : consoleStubs, argv : [ null, null, '-c', 'missing-config' ]},
        callbackStub);
  ...
  it('should support bs-config as function', () => {
    ...
    assert.ok(browserSyncMethodStubs.init.calledWithMatch(
                  {server : {middleware : [ 'm1', 'm2' ]}, b : 2}),
              'configs were merged');
    assert.ok(bsConfigMock.calledWith(bs),
              'browsersync passed into bsconfig fn');
  });
});
